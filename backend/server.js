...
try { db.exec("ALTER TABLE motivation_items    ADD COLUMN wow_goal  TEXT"); } catch {}
try { db.exec("ALTER TABLE motivation_critical ADD COLUMN name      TEXT"); } catch {}
try { db.exec("ALTER TABLE motivation_critical ADD COLUMN indicator TEXT"); } catch {}
...
app.get('/api/pizzerias/:pizzeriaId/motivation2', authMiddleware, requirePizzeriaAccess, requirePerm('motivation'), (req, res) => {
  ...
  const crits  = db.prepare("SELECT id, name, indicator FROM motivation_critical WHERE pizzeria_id = ? ORDER BY sort_order, id").all(pid);
  res.json({
    fund,
    blocks: blocks.map(b => ({
      ...
      items: getIt.all(b.id).map(it => ({
        ...
        weight: it.weight, goal: it.goal,
        has_wow_goal: !!it.has_wow_goal, wow_goal: it.wow_goal ?? null,
      })),
    })),
    critical: crits.map(c => ({ id: c.id, name: c.name ?? c.description ?? '', indicator: c.indicator ?? null })),
  });
});
...
app.post('/api/pizzerias/:pizzeriaId/motivation2/blocks/:blockId/items', authMiddleware, requirePizzeriaAccess, requirePerm('motivation', true), (req, res) => {
  ...
  const { name, weight, goal, has_wow_goal, wow_goal } = req.body;
  if (!name) return res.status(400).json({ error: 'name обязателен' });
  const r  = db.prepare("INSERT INTO motivation_items (block_id, name, weight, goal, has_wow_goal, wow_goal) VALUES (?, ?, ?, ?, ?, ?)").run(block.id, name, weight ?? 0, goal || null, has_wow_goal ? 1 : 0, wow_goal || null);
  const it = db.prepare("SELECT * FROM motivation_items WHERE id = ?").get(r.lastInsertRowid);
  res.status(201).json({ id: it.id, block_id: it.block_id, name: it.name, weight: it.weight, goal: it.goal, has_wow_goal: !!it.has_wow_goal, wow_goal: it.wow_goal ?? null });
});
...
app.put('/api/pizzerias/:pizzeriaId/motivation2/items/:itemId', authMiddleware, requirePizzeriaAccess, requirePerm('motivation', true), (req, res) => {
  ...
  const { name, weight, goal, has_wow_goal, wow_goal } = req.body;
  db.prepare("UPDATE motivation_items SET name=?, weight=?, goal=?, has_wow_goal=?, wow_goal=? WHERE id=?").run(name ?? it.name, weight ?? it.weight, goal ?? null, has_wow_goal ? 1 : 0, wow_goal || null, it.id);
  res.json({ success: true });
});
...
app.post('/api/pizzerias/:pizzeriaId/motivation2/critical', authMiddleware, requirePizzeriaAccess, requirePerm('motivation', true), (req, res) => {
  const { name, indicator } = req.body;
  if (!name) return res.status(400).json({ error: 'name обязателен' });
  const r = db.prepare("INSERT INTO motivation_critical (pizzeria_id, description, name, indicator) VALUES (?, ?, ?, ?)").run(req.pizzeriaId, name, name, indicator || null);
  const c = db.prepare("SELECT id, name, indicator FROM motivation_critical WHERE id = ?").get(r.lastInsertRowid);
  res.status(201).json({ id: c.id, name: c.name ?? '', indicator: c.indicator ?? null });
});
