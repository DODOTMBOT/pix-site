  ...
  function render(): void {
    ...
    const userArea = user
      ? `<div style="display:flex;align-items:center;gap:10px;margin-left:12px;">
           ${isManagement() ? `<button class="btn btn-ghost" style="padding:8px 14px;font-size:13px;" id="header-admin">Панель</button>` : ''}
           <div style="text-align:right;">
             <div style="font-size:13px;font-weight:600;color:#111;">${user.name}</div>
             <div style="font-size:11px;color:#9ca3af;">${roleLabel(user.role)}</div>
           </div>
